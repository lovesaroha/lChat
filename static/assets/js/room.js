...
function showRooms() {
     ...
     if (Object.keys(rooms).length == 0) {
        ...
        <i class="fad fa-comment-alt-lines fa-3x mr-3 icon-primary"></i>
        ...
        <i class="fad fa-comment-alt-smile fa-3x mr-3 icon-primary"></i>
        <div class="w-full self-center"><h3 class="bg-placeholder mb-2 p-2 w-half"></h3><h4 class="bg-placeholder mb-0 p-2"></h4></div></div>
...
function roomTemplate(room) {
     let emailAddress = room.members[0];
     return `<div class="card mb-2 mx-auto max-w-xl flex  p-4">
    <i class="fad fa-comment-alt-lines icon-primary fa-4x mr-3 hide-on-md"></i>
...
function searchRooms(element) {
     ...
     Object.keys(rooms).forEach(_id => {
          // Find matching room.
          let emailAddress = rooms[_id].members[0];
...
function sendJoinRoomRequest(el) {
     ...
     fetch(`/api/join-room`, {
          ...
          body: `{"_id": "${room._id}","password" : "${password}"}`
     }).then(response => response.json()).then(json => {
          activeRoom = json;
          window.location = `/#/room?id=${json._id}`;
          closeModal();
...
function isRoomMember() {
     ...
     for (let i = 0; i < activeRoom.members.length; i++) {
          if (activeRoom.members[i] == user.emailAddress) {
               return true;
          }
     }
     return false;
...
function showMembers() {
     if (!activeRoom._id) { return; }
     let el = document.getElementById("members_id");
     if (el == null) { return; }
     let template = ``;
     for (let i = 0; i < activeRoom.members.length; i++) {
          template += `<div class="bg-primary shadow-xl mb-2 p-2"><h4 class="mb-0 text-white"><i class="fad fa-circle"></i> ${activeRoom.members[i]}</h4></div>`
     }
     el.innerHTML = `${template}`;
}

// Handle room unavailable.
function handleRoomUnavailable(message) {
     if (activeRoom._id || !rooms[message._id]) { return; }
     delete rooms[message._id];
     showRooms();
}